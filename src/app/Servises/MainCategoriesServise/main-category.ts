...
export class MainCategoryServise {
  ...
  // POST: api/MainCategories/admin
  addMainCategory(category: ManagerMainCategoryModel): void {
    this.http.post<MainCategoriesModel>(`${this.BASIC_URL}/admin`, category).subscribe({
      next: () => this.getMainCategory(), // רענון הנתונים לאחר הצלחה
      error: (err) => this.errorSubject.next(err)
    });
  }

  // PUT: api/MainCategories/admin/{id}
  updateMainCategory(id: number, category: MainCategoriesModel): void {
    this.http.put<void>(`${this.BASIC_URL}/admin/${id}`, category).subscribe({
      next: () => this.getMainCategory(), // רענון הנתונים לאחר הצלחה
      error: (err) => this.errorSubject.next(err)
    });
  }

  // DELETE: api/MainCategories/admin/{id}
  deleteMainCategory(id: number): void {
    this.http.delete<void>(`${this.BASIC_URL}/admin/${id}`).subscribe({
      next: () => this.getMainCategory(), // רענון הנתונים לאחר הצלחה
      error: (err) => this.errorSubject.next(err)
    });
  }
